#include "Objeto.h"
#include "clipping.h" // Inclui o NOVO clipper 2D
#include <cmath>
...
Ponto4D Reta::getCentroGeometrico() const {
    ...
    Ponto4D c = (pontos[0] + pontos[1]); c.x /= 2.0; c.y /= 2.0; c.z /= 2.0; c.w = 1.0; return c;
}
...
Ponto4D Poligono::getCentroGeometrico() const {
    if (pontos.isEmpty()) return Ponto4D(0, 0, 0);
    Ponto4D s(0, 0, 0, 0); for (const Ponto4D& p : pontos) s = s + p;
    s.x /= pontos.size(); s.y /= pontos.size(); s.z /= pontos.size(); s.w = 1.0;
    return s;
}
...
void CurvaBezier::desenhar(QPainter& pintar, const Matriz4x4& matrizFinal, const QRectF& limites) {
    ...
    if(this->isSelecionado()){ pintar.setPen(QPen(Qt::yellow, 2)); }
    else {
        pintar.setPen(QPen(m_cor, 2));
    }
